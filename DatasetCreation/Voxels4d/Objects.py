...
class Sphere(object):
    ...
    def __init__(self, full_grid, center, radius, size):
        ...
        self.create_sphere()
        grid_dilation = scipy.ndimage.binary_dilation(self.grid, iterations=2)
        # Check intersect
    ...
    def random(cls, grid, size):
        center = (
            random.randrange(1, size - 1, 1),
            random.randrange(1, size - 1, 1),
            random.randrange(1, size - 1, 1),
            random.randrange(1, size - 1, 1),
        )
        radius = random.randrange(5, 7, 1)
        return cls(grid, center, radius, size)

    def create_sphere(self):
        x, y, z, w = np.indices((self.size, self.size, self.size, self.size))
        self.grid = (
            (pow(x - self.center[0], 2) / pow(self.radius, 2))
            + (pow(y - self.center[1], 2) / pow(self.radius, 2))
            + (pow(z - self.center[2], 2) / pow(self.radius, 2))
            + (pow(w - self.center[3], 2) / pow(self.radius, 2))
        ) <= 1
