...
class Shape(ABC):
    ...
    def _get_intersecting_vector(self):
        ...
        intersecting_vector = [
            (size - 2 - x) if (x >= size - 1) else (-x + 1) if (x <= 0) else 0
        ...
        intersection = (
            scipy.ndimage.binary_dilation(
                self.grid, scipy.ndimage.generate_binary_structure(3, 3), 1
            )
            & self.full_grid
        )
    ...
    def _place_and_move(self):
        ...
        while intersecting_vector is not None:
            ...
            self.center = [
                self.center[0] + intersecting_vector[0],
                self.center[1] + intersecting_vector[1],
                self.center[2] + intersecting_vector[2],
            ]
            self._create_grid()
